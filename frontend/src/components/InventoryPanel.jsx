...
export default function InventoryPanel({ parts, form, editingId, onChange, onSubmit, onEdit, onDelete, onCancelEdit, onSell, onReadLocalHardware, onPrintLocalHardware, onRegisterAndPrintLocalHardware }) {
        ...
        <table>
          ...
          <tbody>
            {visibleParts.map((part) => (
              <tr key={part.id}>
                <td>{part.codigo || part.codigo_sku}</td>
                <td><button className="table-link" type="button" onClick={() => setDetailsItem(part)}>{part.nome}</button></td>
                <td>{part.categoria}</td>
                <td>
                  <span className={`status-pill ${part.quantidade_estoque <= (part.quantidade_minima || 0) ? 'risk' : 'ok'}`}>
                    {part.quantidade_estoque}
                  </span>
                </td>
                <td>{part.quantidade_minima}</td>
                <td>
                  <div className="table-actions">
                    <button className="secondary-button" type="button" onClick={() => onEdit(part)}>Editar</button>
                    <button className="ghost-button" type="button" onClick={() => onDelete(part.id)}>Excluir</button>
                  </div>
                </td>
              </tr>
            ))}
            {!visibleParts.length && <tr><td colSpan="6"><p className="empty-state">Nenhum produto sem preço pendente.</p></td></tr>}
          </tbody>
        </table>
      </div>
      {hardwareModalOpen && <LocalHardwareModal onClose={() => setHardwareModalOpen(false)} onRead={onReadLocalHardware} onPrint={onPrintLocalHardware} onRegisterAndPrint={onRegisterAndPrintLocalHardware} />}
