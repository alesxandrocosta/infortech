...
const columns = ['Recebido', 'Em Análise', 'Aguardando Peça', 'Aprovado', 'Concluído', 'Entregue', 'Retorno Assistência', 'Desistência do Cliente'];

export default function OrderPanel({ orders, form, services, parts, checklist, editingId, labelQuantities, onChange, onSubmit, onEdit, onDelete, onCancelEdit, onPrintLabels, onProgressUpdate, onChecklistStatusChange, onChecklistAddItem, onChecklistRemoveItem, onLoadHistory }) {
  ...
  return (
    <section className="panel form-panel order-workspace">
      <div className="panel-header"><div><span className="eyebrow">Operação diária</span><h2>Ordens de serviço</h2><p className="panel-subtitle">Acompanhe responsáveis, status e próximas ações.</p></div><button className="primary-button" type="button" onClick={() => { onCancelEdit(); setIsFormOpen(true); }}>+ Nova ordem</button></div>
      ...
      <div className="orders-list">
        {orders.map((order) => <article className="order-row" key={order.id} onClick={() => openDetails(order)}>
          ...
          <div className="table-actions"><button className="primary-button" type="button" onClick={(event) => { event.stopPropagation(); openDetails(order); }}>Ver detalhes</button><button className="secondary-button" type="button" onClick={(event) => { event.stopPropagation(); onEdit(order); setIsFormOpen(true); }}>Editar</button><button className="secondary-button" type="button" onClick={(event) => { event.stopPropagation(); onPrintLabels(order, labelQuantities[order.id] || 4); }}>Etiquetas</button><button className="ghost-button" type="button" onClick={(event) => { event.stopPropagation(); onDelete(order.id); }}>Excluir</button></div>
        </article>)}
      ...
      {isFormOpen && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeForm(); }}>
        <div className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="order-modal-title">
          ...
          <form className="entity-form" onSubmit={async (event) => { const saved = await onSubmit(event); if (saved !== false) setIsFormOpen(false); }}>
            ...
            <div className="order-composition">
              <div className="composition-block services-picker"><div className="composition-title">Serviços da OS</div><input className="service-search" value={serviceQuery} onChange={(event) => setServiceQuery(event.target.value)} placeholder="Buscar por nome, SKU ou categoria" />{Object.entries(serviceGroups).map(([category, categoryServices]) => <details className="service-submenu" key={category} open><summary>{category} <span>{categoryServices.length}</span></summary>{categoryServices.map((service) => <label className="selection-row" key={service.id}><input type="checkbox" checked={form.service_ids?.includes(service.id) || false} onChange={() => toggleService(service.id)} /><span>{service.nome} · {service.modalidade || 'Presencial'}<small>{service.codigo_sku || service.sku || `ID ${service.id}`}</small></span><strong>R$ {Number(service.preco_sugerido || 0).toFixed(2)}</strong></label>)}</details>)}{!filteredServices.length && <p className="empty-state">Nenhum serviço encontrado.</p>}</div>
              <div className="composition-block"><div className="composition-title">Peças disponíveis</div>{parts.map((part) => { const selected = form.part_items?.find((item) => item.id === part.id); return <label className={`selection-row ${selected ? 'selection-row-selected' : ''}`} key={part.id}><input type="checkbox" checked={Boolean(selected)} onChange={() => togglePart(part.id)} /><span>{part.nome}<small>{part.quantidade_estoque ?? 0} em estoque</small></span>{selected && <input className="part-quantity" type="number" min="1" value={selected.quantidade} onChange={(event) => changePartQuantity(part.id, event.target.value)} />}<strong>R$ {Number(part.preco_venda || 0).toFixed(2)}</strong></label>; })}</div>
              <div className="order-totals"><span>Análise: <strong>R$ 120,00</strong></span><span>Serviços: <strong>R$ {servicesTotal.toFixed(2)}</strong></span><span>Peças: <strong>R$ {partsTotal.toFixed(2)}</strong></span><span className="approved-total">Total se aprovado: <strong>R$ {approvedTotal.toFixed(2)}</strong></span></div>
            </div>
